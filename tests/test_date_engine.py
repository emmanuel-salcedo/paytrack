...
def test_cycle_boundaries_around_anchor_payday() -> None:
    ...
    assert cycle_on_payday == PayCycle(start=date(2026, 1, 2), end=date(2026, 1, 15))

    cycle_day_before = cycle_for_date(date(2026, 1, 14), ANCHOR)
    assert cycle_day_before == cycle_on_payday

    cycle_day_after = cycle_for_date(date(2026, 1, 16), ANCHOR)
    assert cycle_day_after == PayCycle(start=date(2026, 1, 16), end=date(2026, 1, 29))


def test_cycle_is_inclusive_and_always_14_days() -> None:
    cycle = cycle_for_date(date(2026, 2, 11), ANCHOR)
    assert cycle.start == date(2026, 1, 30)
    assert cycle.end == date(2026, 2, 12)
    assert (cycle.end - cycle.start).days == 13
    assert cycle.contains(date(2026, 1, 30))
    assert cycle.contains(date(2026, 2, 12))
    assert not cycle.contains(date(2026, 1, 29))
    assert not cycle.contains(date(2026, 2, 13))
...
def test_payday_cadence_matches_every_14_days_from_anchor_long_range() -> None:
    ...
    assert cycle.end.weekday() == 3
    assert ((cycle.end - ANCHOR).days % 14) == 0
